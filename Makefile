...
validate: validate-network-app validate-endpoint-tests validate-endpoint-local validate-logs validate-traces validate-v1-network-app validate-v1-endpoint-tests
...
validate-v1-network-app:
	$(DOCKER_WEAVER) registry check -r "$(V1_NETWORK_APP_REGISTRY)" --future

validate-v1-endpoint-tests:
	$(DOCKER_WEAVER) registry check -r "$(V1_ENDPOINT_TESTS_REGISTRY)" --future
...
docs-generate-v1-network-app:
	$(DOCKER_WEAVER) registry generate --registry="$(V1_NETWORK_APP_REGISTRY)" --templates="$(SEMCONV_TEMPLATES_ROOT)" markdown "$(V1_NETWORK_APP_DOCS)" --future

docs-generate-v1-endpoint-tests:
	$(DOCKER_WEAVER) registry generate --registry="$(V1_ENDPOINT_TESTS_REGISTRY)" --templates="$(SEMCONV_TEMPLATES_ROOT)" markdown "$(V1_ENDPOINT_TESTS_DOCS)" --future

docs-generate-metrics: docs-generate-metrics-network-app docs-generate-metrics-endpoint-tests docs-generate-metrics-endpoint-local docs-generate-metrics-v1-network-app docs-generate-metrics-v1-endpoint-tests
...
docs-generate-metrics-v1-network-app:
	$(DOCKER_WEAVER) registry generate --registry="$(V1_NETWORK_APP_REGISTRY)" --templates="$(LOCAL_TEMPLATES_ROOT)" "$(LOCAL_METRICS_TARGET)" "$(V1_NETWORK_APP_DOCS)" --future
...
docs-update-inline: templates docs-update-inline-network-app docs-update-inline-endpoint-tests docs-update-inline-endpoint-local docs-update-inline-logs docs-update-inline-traces docs-update-inline-v1-network-app docs-update-inline-v1-endpoint-tests
...
docs-update-inline-v1-network-app:
	$(DOCKER_WEAVER) registry update-markdown --registry="$(V1_NETWORK_APP_REGISTRY)" --templates="$(SEMCONV_TEMPLATES_ROOT)" --target=markdown --future "$(V1_NETWORK_APP_DOCS)"

docs-update-inline-v1-endpoint-tests:
	$(DOCKER_WEAVER) registry update-markdown --registry="$(V1_ENDPOINT_TESTS_REGISTRY)" --templates="$(SEMCONV_TEMPLATES_ROOT)" --target=markdown --future "$(V1_ENDPOINT_TESTS_DOCS)"
