#!/usr/bin/env python3
"""Add a stable event specs link to generated logs registry READMEs."""
...
EVENT_SPECS_LINK_LABEL = "Event Specs"


def event_specs_link(path: pathlib.Path) -> str:
    section_name = path.parent.name
    return f"- [{EVENT_SPECS_LINK_LABEL}]({section_name}-events.md)\n"


def ensure_event_specs_link(path: pathlib.Path) -> bool:
    content = path.read_text(encoding="utf-8")
    has_entities_page = (path.parent / "entities" / "README.md").exists()
    specs_link = event_specs_link(path)
...
def main(argv: list[str]) -> int:
    if len(argv) < 2:
        print("usage: link_event_specs_in_registry_readme.py <README.md> [<README.md> ...]")
        return 1

    for raw_path in argv[1:]:
        readme_path = pathlib.Path(raw_path)
        changed = ensure_event_specs_link(readme_path)
